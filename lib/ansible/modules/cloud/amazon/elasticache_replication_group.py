...
options:
  ...
  security_groups:
    description:
      - "One or more Amazon VPC security group names or IDs associated with this replication group. Use this parameter only when you are creating a replication group in an Amazon Virtual Private Cloud (VPC)."
      required: false
      default: null
  tags:
    description:
...
def create_or_update_rep_group(module, connection, connection_ec2):
    """
    Create or update an Elasticache replication group

    :param module:
    :param connection:
    :param connection_ec2:
    :return:
    """
    ...
    params['SecurityGroupIds'] = get_ec2_security_group_ids_from_names(module.params.get('security_group_ids'), connection_ec2, boto3=True)
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            security_groups=dict(default=None, required=False, type='list'),
    ...
    try:
        ...
        connection = boto3_conn(module, conn_type='client', resource='elasticache', region=region, endpoint=ec2_url, **aws_connect_params)
    ...
    if state == "present":
        create_or_update_rep_group(module, connection, connection_ec2)
    elif state == "absent":
